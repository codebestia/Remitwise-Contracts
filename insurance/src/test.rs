...
fn test_create_policy_success() {
    ...
    let id = client.create_policy(&caller, &String::from_str(&env, "P1"), &health_str(&env), &5_000_000i128, &50_000_000i128, &None);
    assert_eq!(id, 1);
...
fn test_pagination() {
    ...
    for i in 0..10 {
        client.create_policy(&owner, &String::from_str(&env, "P"), &health_str(&env), &5_000_000i128, &50_000_000i128, &None);
    }
...
fn test_total_premium_isolation() {
    ...
    client.create_policy(&u1, &String::from_str(&env, "P1"), &health_str(&env), &5_000_000i128, &50_000_000i128, &None);
    client.create_policy(&u2, &String::from_str(&env, "P2"), &health_str(&env), &6_000_000i128, &50_000_000i128, &None);
    assert_eq!(client.get_total_monthly_premium(&u1), 5_000_000);
    assert_eq!(client.get_total_monthly_premium(&u2), 6_000_000);
}
...
fn test_batch_pay() {
    ...
    client.init(&Address::generate(&env));
    let owner = Address::generate(&env);
    let id1 = client.create_policy(&owner, &String::from_str(&env, "P1"), &health_str(&env), &5_000_000i128, &50_000_000i128, &None);
    let id2 = client.create_policy(&owner, &String::from_str(&env, "P2"), &health_str(&env), &5_000_000i128, &50_000_000i128, &None);
